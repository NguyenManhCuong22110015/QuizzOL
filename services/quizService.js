    ...
    async updateImageQuiz(quizId, imageUrl,public_id) {
        try {
            const [media] = await db('media')
                .insert({ url: imageUrl,resource_type: 'IMAGE', public_id: public_id })
            await db('quiz')
                .where('id', quizId)
                .update({ media: media });
        }
        ...
        try {
            ...
            const quizzes = await db('quiz')
                ...
                .select(
                    ...
                    'quiz.media',
                    'media.url as mediaUrl',  // Select the URL from media table
                    'category.name as categoryName',
                    db.raw('GROUP_CONCAT(DISTINCT tag.name) as tagsString')
                )
                .leftJoin('category', 'quiz.category', 'category.id')
                .leftJoin('media', 'quiz.media', 'media.id')  // Join with media table
